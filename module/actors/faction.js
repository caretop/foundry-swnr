...
export class SWNRFactionActor extends SWNRBaseActor {
    getRollData() {
    ...
    async logMessage(title, content, longContent = null, logRollString = null) {
        ...
        const gm_ids = ChatMessage.getWhisperRecipients("GM")
            ...
            .map((i) => i.id)
            .filter((i) => i !== null);
        if ((_b = (_a = game.modules) === null || _a === void 0 ? void 0 : _a.get("foundryvtt-simple-calendar")) === null || _b === void 0 ? void 0 : _b.active) {
    ...
    async addTag(name) {
        ...
        if (!match) {
            (_a = ui.notifications) === null || _a === void 0 ? void 0 : _a.error(`Error unable to find tag ${name}`);
            return;
        }
        const tags = this.data.data.tags;
        tags.push(match[0]);
    ...
    async addBase(hp, assetType, name, imgPath) {
        ...
        if (hp > this.data.data.health.max) {
            (_a = ui.notifications) === null || _a === void 0 ? void 0 : _a.error(`Error HP of new base (${hp}) cannot be greater than faction max HP (${this.data.data.health.max})`);
            return;
        }
        if (hp > this.data.data.facCreds) {
            (_b = ui.notifications) === null || _b === void 0 ? void 0 : _b.error(`Error HP of new base (${hp}) cannot be greater than facCred  (${this.data.data.facCreds})`);
            return;
        }
    ...
    async startTurn() {
        ...
        */
        ...
        const assetMaintTotal = assetWithMaint
            ...
            .reduce((i, n) => i + n, 0);
        const cunningAssetsOverLimit = Math.min(this.data.data.cunningRating - this.data.data.cunningAssets.length, 0);
        const forceAssetsOverLimit = Math.min(this.data.data.forceRating - this.data.data.forceAssets.length, 0);
        const wealthAssetsOverLimit = Math.min(this.data.data.wealthRating - this.data.data.wealthAssets.length, 0);
        const costFromAssetsOver = cunningAssetsOverLimit + forceAssetsOverLimit + wealthAssetsOverLimit;
        const income = wealthIncome +
            cunningIncome +
            forceIncome +
            assetIncome -
            assetMaintTotal +
        ...
        let msg = `<b>Income this round: ${income}</b>.<br> From ratings: ${wealthIncome + cunningIncome + forceIncome} (W:${wealthIncome} C:${cunningIncome} F:${forceIncome})<br>From assets: ${assetIncome}.<br>Maintenance -${assetMaintTotal}.<br>`;
    ...
    async setGoal() {
        var _a;
        (_a = ui.notifications) === null || _a === void 0 ? void 0 : _a.info("set goal");
    }
    async ratingUp(type) {
        var _a, _b, _c;
        ...
        if (ratingLevel == 8) {
            (_a = ui.notifications) === null || _a === void 0 ? void 0 : _a.info("Rating is already at max");
            return;
        }
        if (!ratingLevel) {
            ratingLevel = 0;
        ...
        if (targetLevel in HEALTH__XP_TABLE) {
            ...
            if (req_xp > xp) {
                (_b = ui.notifications) === null || _b === void 0 ? void 0 : _b.error(`Not enough XP to raise rating. Have ${xp} Need ${req_xp}`);
                return;
            }
            xp -= req_xp;
            if (type == "cunning") {
            ...
            (_c = ui.notifications) === null || _c === void 0 ? void 0 : _c.info(`Raised ${type} rating to ${targetLevel} using ${xp} xp`);
        }
    }
    async setHomeWorld(journalId) {
        var _a, _b, _c;
        const journal = (_a = game.journal) === null || _a === void 0 ? void 0 : _a.get(journalId);
        if (!journal || !journal.name) {
            (_b = ui.notifications) === null || _b === void 0 ? void 0 : _b.error("Cannot find journal");
            return;
        }
        ...
        (_c = ui.notifications) === null || _c === void 0 ? void 0 : _c.error("TODO create asset base with max health.");
        await this.update({ data: { homeworld: journal.name } });
    }
    prepareDerivedData() {
        const data = this.data.data;
        const assets = (this.items.filter((i) => i.type == "asset"));
        const cunningAssets = assets.filter((i) => i.data.data["assetType"] === "cunning");
        const forceAssets = assets.filter((i) => i.data.data["assetType"] === "force");
        const wealthAssets = assets.filter((i) => i.data.data["assetType"] === "wealth");
...
export const FACTION_GOALS = [
    {
        ...
        desc: "Destroy a number of Force assets of rival factions equal to your faction`s Force rating. Difficulty is 1/2 number of assets destroyed.",
    },
    {
        name: "Commercial Expansion",
        desc: "Destroy a number of Wealth assets of rival factions equal to your faction`s Wealth rating. Difficulty is 1/2 number of assets destroyed.",
    },
    {
        name: "Intelligence Coup",
        desc: "Destroy a number of Cunning assets of rival factions equal to your faction`s Cunning rating. Difficulty is 1/2 number of assets destroyed.",
    },
    {
        name: "Planetary Seizure",
        desc: "Take control of a planet, becoming the legitimate planetary government. Difficulty equal to half the average of the current ruling faction`s Force, Cunning, and Wealth ratings. If the planet somehow lacks any opposing faction to resist the seizure, it counts as Difficulty 1.",
    },
    {
        name: "Expand Influence",
        desc: "Plant a Base of Influence on a new planet. Difficulty 1, +1 if the attempt is contested by a rival faction.",
    },
    {
        name: "Blood the Enemy",
        desc: "Inflict a number of hit points of damage on enemy faction assets or bases equal to your faction`s total Force, Cunning, and Wealth ratings. Difficulty 2.",
    },
    {
