...
export class SWNRNPCActor extends SWNRBaseActor {
    ...
    async rollSave(save) {
        ...
        await roll.roll({ async: true });
        const flavor = game.i18n.format(parseInt(roll.result) >= this.data.data.saves
            ? game.i18n.localize("swnr.npc.saving.success")
            : game.i18n.localize("swnr.npc.saving.failure"), { actor: this.name, target: this.data.data.saves });
        roll.toMessage({ flavor, speaker: { actor: this.id } });
    }
    _onCreate(data, options, userId) {
        var _a;
        super._onCreate(data, options, userId);
        if (((_a = game.user) === null || _a === void 0 ? void 0 : _a.isGM) && game.userId === userId) {
            if (this.data["items"]["length"] || game.userId !== userId)
                return;
            const unarmed = this.data.items.filter((i) => i.name == game.i18n.localize("swnr.npc.unarmed"));
            if (unarmed.length == 0) {
...
Hooks.on("createToken", (document, options, userId) => {
    ...
    if (game.settings.get("swnr", "useRollNPCHD")) {
        if (((_a = game.user) === null || _a === void 0 ? void 0 : _a.isGM) && game.userId === userId) {
            if (((_b = document.actor) === null || _b === void 0 ? void 0 : _b.type) == "npc") {
